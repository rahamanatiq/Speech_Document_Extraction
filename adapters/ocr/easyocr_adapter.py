...
# --- Structured meta-field extraction ---
#
# Each pattern requires an explicit label (a keyword immediately followed
# by ":" or "-") before it will extract anything. This is a deliberate
# design choice consistent with the "never guess" principle used
# elsewhere: a labeled field ("Age: 45") is a confident read; an unlabeled
# number floating near the top of the page is not, and is left out of meta
# entirely rather than guessed at. Column-header rows like "Test Name
# Value Unit" are naturally excluded because they lack the ":"/"-"
# delimiter these patterns require.
_META_LABEL_PATTERNS: dict[str, re.Pattern] = {
    "patient_name": re.compile(r"(?:patient(?:\s*name)?|name)\s*[:\-]\s*(.+)", re.IGNORECASE),
    "age": re.compile(r"\bage\s*[:\-]\s*([0-9]{1,3}\s*(?:y|yrs?|years?)?)", re.IGNORECASE),
    "sex": re.compile(r"\b(?:sex|gender)\s*[:\-]\s*(male|female|m|f)\b", re.IGNORECASE),
    "report_date": re.compile(r"(?:report\s*date|date)\s*[:\-]\s*(.+)", re.IGNORECASE),
    "lab_name": re.compile(r"(?:lab(?:oratory)?(?:\s*name)?)\s*[:\-]\s*(.+)", re.IGNORECASE),
    "reference_no": re.compile(r"(?:ref(?:erence)?\.?\s*(?:no\.?|number)?|report\s*id)\s*[:\-]\s*(\S+)", re.IGNORECASE),
}


def _extract_meta(lines: list[str]) -> dict[str, str]:
    """Best-effort header extraction into the six fields the API contract
    requires (patient_name, age, sex, report_date, lab_name,
    reference_no). A field is only populated when a clear label is found;
    otherwise it's simply absent from the dict — never guessed at.
    """
    meta: dict[str, str] = {}
    for line in lines:
        stripped = line.strip()
        for field_name, pattern in _META_LABEL_PATTERNS.items():
            if field_name in meta:
                continue  # first confident match wins, don't overwrite with a weaker one
